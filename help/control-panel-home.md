...
控制面板现在可供具有混合托管模型的客户使用。 这些客户可以通过提供在其控制面板实例中配置的MID/RT实例URL(在控制面板中)来利用的功能。 [了解更多信息](instances-settings/using/external-accounts.md)


**吞吐量和延迟监控更新**

吞吐量和延迟监控功能已得到增强。 [了解更多信息](performance-monitoring/using/thoughputs-latencies.md)

* 您现在可以识别对实例吞吐量有贡献的前5个投放的ID。
* Campaign Classicv7/v8客户现在可以查看特定渠道的延迟。
