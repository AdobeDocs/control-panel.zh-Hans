...
1. 在 **[!UICONTROL 子域和证书]** 卡片上，选择所需的生产实例，然后单击 **[!UICONTROL 设置新子域]**.

   ![](assets/subdomain1.png)

1. 单击 **[!UICONTROL 下一个]** 以确认完全委派方法。
...
1. 在使用相应的Adobe名称服务器信息创建子域后，单击 **[!UICONTROL 下一个]**.
...
1. 要将子域的SSL证书的生成委派给Adobe，请启用 **[!UICONTROL 子域选择使用Adobe托管SSL]** 选项。 [了解有关SSL证书委派的更多信息](delegate-ssl.md)
...
1. 选择 **[!UICONTROL CNAME]** 方法，然后单击 **[!UICONTROL 下一个]**.
...
1. 输入您在托管解决方案中创建的子域。
   ...
   要将子域的SSL证书的生成委派给Adobe，请启用 **[!UICONTROL 子域选择使用Adobe托管SSL]** 选项。 [了解有关SSL证书委派的更多信息](delegate-ssl.md)
...
1. 确保前面步骤中的所有 DNS 记录均已生成至您的域托管解决方案中。如果一切配置正确，请选择第一条语句，然后单击 **[!UICONTROL 下一个]** 以确认。

   如果要创建记录并稍后提交子域配置，请选择第二条语句。 然后，您就可以直接从子域管理屏幕恢复子域配置 **[!UICONTROL 正在处理]** 区域。 请注意，要放在您的服务器上的 DNS 记录将由控制面板保存 30 天。在这之后，您必须从头开始配置子域。

   >[!NOTE]
   >
   >如果您选择不将SSL证书委派给Adobe，则这是子域配置的最后一步。 单击 **[!UICONTROL 提交]** 按钮。
...
1. 请确保所有证书记录均已生成至您的域托管解决方案中。如果一切配置正确，请选择第一条语句，然后单击 **[!UICONTROL 提交]** 以确认。
...
1. 如果检查成功，控制面板将开始设置包含 DNS 记录、其他 URL、收件箱等的子域。
   ...
   您可以通过单击子域配置获取有关配置进度的更多详细信息 **[!UICONTROL 详细信息]** 按钮。
...
您可以通过单击 **[!UICONTROL 子域详细信息]** 和 **[!UICONTROL 发件人信息]** 按钮。
...
* 在某些情况下，子域配置会完成，但子域可能无法成功验证。子域将保留在 **[!UICONTROL 已配置]** 带有作业日志的列表，其中提供有关错误的信息。 如果您在解决此问题时遇到麻烦，请联系客户关怀团队。
* 如果子域在配置后显示为“未验证”，则启动新的子域验证(**...** / **[!UICONTROL 验证子域]**)。 如果它仍显示相同的状态，原因可能是对收件人模式进行了一些自定义操作，无法使用标准流程进行验证。请尝试使用该子域发送活动。
